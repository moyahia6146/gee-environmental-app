...
def ee_authenticate():
    try:
        ...
        scoped_credentials = credentials.with_scopes(['[https://www.googleapis.com/auth/earthengine](https://www.googleapis.com/auth/earthengine)'])
...
try:
    s2 = ee.ImageCollection("COPERNICUS/S2_SR_HARMONIZED").filterBounds(roi).filterDate(start_date_str, end_date_str).filter(ee.Filter.lt('CLOUDY_PIXEL_PERCENTAGE', cloud_cover)).median().clip(roi)
    ...
    add_map_elements(m1, "Sentinel-2 Natural Color")
...
try:
    ...
    add_map_elements(m2, "Topographic Map")
...
try:
    modis = ee.ImageCollection("MODIS/061/MOD13Q1").filterBounds(roi).filterDate(start_date_str, end_date_str).select('NDVI').median().multiply(0.0001).clip(roi)
    degradation = ee.Image(1).where(modis.lt(0.2), 3).where(modis.gte(0.2).And(modis.lt(0.5)), 2).where(modis.gte(0.5), 1).clip(roi)
    m3 = geemap.Map(center=[lat, lon], zoom=12)
    m3.addLayer(degradation, {'min': 1, 'max': 3, 'palette': ['00FF00', 'FFFF00', 'FF0000']}, "Land Degradation")
    add_map_elements(m3, "Land Degradation")
...
try:
    s1 = ee.ImageCollection("COPERNICUS/S1_GRD").filterBounds(roi).filterDate(start_date_str, end_date_str).filter(ee.Filter.listContains('transmitterReceiverPolarisation', 'VV')).select('VV').median().clip(roi)
    moisture_class = ee.Image(1).where(s1.lt(-15), 4).where(s1.gte(-15).And(s1.lt(-10)), 3).where(s1.gte(-10).And(s1.lt(-5)), 2).where(s1.gte(-5), 1).clip(roi)
    m4 = geemap.Map(center=[lat, lon], zoom=12)
    m4.addLayer(moisture_class, {'min': 1, 'max': 4, 'palette': ['0000FF', '00FFFF', 'FFA500', '8B4513']}, "Soil Moisture")
    add_map_elements(m4, "Soil Moisture")
...
try:
    ...
    lulc = worldcover.remap([10, 20, 30, 40, 50, 60, 70, 80, 90, 95, 100], [1, 2, 2, 3, 4, 5, 5, 1, 1, 1, 1]).clip(roi)
    m5 = geemap.Map(center=[lat, lon], zoom=12)
    m5.addLayer(lulc, {'min': 1, 'max': 5, 'palette': ['228B22', 'ADFF2F', 'FFD700', 'A9A9A9', 'D2B48C']}, "LULC")
    add_map_elements(m5, "LULC")
...
try:
    l7_2000 = ee.ImageCollection("LANDSAT/LE07/C02/T1_L2").filterBounds(roi).filterDate('2000-01-01', '2000-12-31').median()
    ndvi_2000 = l7_2000.normalizedDifference(['SR_B4', 'SR_B3']).rename('NDVI')
    l8_2025 = ee.ImageCollection("LANDSAT/LC08/C02/T1_L2").filterBounds(roi).filterDate('2024-01-01', '2025-12-31').median()
    ndvi_2025 = l8_2025.normalizedDifference(['SR_B5', 'SR_B4']).rename('NDVI')
    ndvi_diff = ndvi_2025.subtract(ndvi_2000).clip(roi)
    change_class = ee.Image(2).where(ndvi_diff.lt(-0.1), 1).where(ndvi_diff.gt(0.1), 3).clip(roi)
    m6 = geemap.Map(center=[lat, lon], zoom=12)
    m6.addLayer(change_class, {'min': 1, 'max': 3, 'palette': ['FF0000', '808080', '00FF00']}, "Vegetation Change")
    add_map_elements(m6, "Vegetation Change")
